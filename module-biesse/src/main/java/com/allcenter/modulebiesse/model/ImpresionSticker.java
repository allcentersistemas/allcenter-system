...
public class ImpresionSticker {
    ...
    /** Empleado que disparó la impresión (resuelto vía AuthGateway con el JWT). */
    @Column(name = "usuarioid", nullable = false)
    private Long usuarioId;

    /** Orden Biesse a la que pertenecen las piezas impresas. */
    @Column(name = "orderid")
    private Long orderId;

    /** MANUAL / AUTOMATICO / REIMPRESION (etc). */
    @Column(name = "metodo", length = 32)
    private String metodo;

    /** Identificador de equipo/PC/etiquetadora reportado por el cliente. */
    @Column(name = "equipo", length = 128)
    private String equipo;

    /** Sucursal / ruta / pantalla desde la que se imprimió. */
    @Column(name = "ubicacion", length = 128)
    private String ubicacion;

    /** IP del cliente que disparó la solicitud (resuelta server-side). */
    ...
    /** Cantidad total de etiquetas impresas en este evento. */
    @Column(name = "cantidad_etiquetas", nullable = false)
    private Integer cantidadEtiquetas = 0;

    /** Comentario libre opcional (ej. "reemplazo por daño"). */
